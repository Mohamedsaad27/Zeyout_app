        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-striped table-bordered">
                    <thead>
                        <tr>
                            <th>Brand English Name</th>
                            <th>Brand Arabic Name</th>
                            <th>Logo</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="ms-2">
                                            <h6 class="fw-semibold mb-0">{{ $brand->name_en }}</h6>
                                        </div>
                                    </div>
                                </td>
                                <td>{{ $brand->name_ar }}</td>
                                <td>
                                    <img src="{{ $brand->logo }}" alt="Brand Logo" class="img-fluid" style="max-width: 100px; max-height: 100px;">
                                </td>
                                <td>
                                    <div class="action-btn d-flex">
                                        <a href="{{ route('brands.show', $brand->id) }}" class="text-info edit me-2">
                                            <i class="ti ti-eye fs-5"></i>
                                        </a>
                                        <a href="{{ route('brands.edit', $brand->id) }}" class="text-primary edit me-2">
                                            <i class="ti ti-edit fs-5"></i>
                                        </a>
                                        <form action="{{ route('brands.destroy', $brand->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" class="bg-transparent border-0 delete-brand">
